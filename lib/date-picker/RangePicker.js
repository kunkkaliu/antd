...
var RangePicker = (_temp = _class = function (_React$Component) {
  ...
  RangePicker.prototype.render = function render() {
    ...
    if (props.showTime && props.showTime.disabledHours) {
        const disabledHours = props.showTime.disabledHours();
        const hours = [];
        for(var i = 0; i < 24; i++) {
            if(disabledHours.indexOf(i) < 0) {
                hours.push(i);
            }
        }
        (hours.length == 0) && hours.push(0);
        const disabledMinutes = props.showTime.disabledMinutes ? props.showTime.disabledMinutes(hours[0]) : [];
        const minutes = [];
        for(var i = 0; i < 60; i++) {
            if(disabledMinutes.indexOf(i) < 0) {
                minutes.push(i);
            }
        }
        (minutes.length == 0) && minutes.push(0);
        var now = new Date();
        now.setHours(hours[0]);
        now.setMinutes(minutes[0]);
        defaultCalendarValue.setTime(now.getTime());
    }else {
